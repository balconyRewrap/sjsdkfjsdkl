...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run inference with a from-scratch transformer model.")
    parser.add_argument("--model-dir", type=Path, default=Path("models/scratch-transformer-sentiment"))
...
def main() -> None:
    ...
    weights_path = args.model_dir / "model.pt"
    if not config_path.exists() or not vocab_path.exists() or not weights_path.exists():
        raise SystemExit(f"Missing model artifacts in {args.model_dir}")
